...
awaitlingo uses macOS Automation to identify the frontmost app and activate a coding app later. It does not use Accessibility.

## First-run dialog

1. Install awaitlingo and submit a prompt that runs longer than `delaySeconds`.
...
The lesson URL may still open, but awaitlingo may be unable to capture or reactivate the previous app.

To re-enable it:

1. Open **System Settings**.
2. Go to **Privacy & Security → Automation**.
3. Find the terminal or host app that ran awaitlingo.
4. Enable the relevant target, such as System Events or your coding app.
5. Run `awaitlingo status`, then try another prompt longer than the configured delay.
...
awaitlingo never sends keystrokes, clicks UI controls, or reads screen contents. Do not grant Accessibility access for awaitlingo. URL opening uses the system `open` command; app detection and reactivation use Apple events.
